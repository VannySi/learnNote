...
Page({
  ...
  onLoad:function(options){
    ...
    that.setData({
      ...
      cities: that.data.provinces[that.data.province],
  ...
  provinceChange: function(e){
    ...
    that.setData({
      cities: that.data.provinces[val[1]].cities,
      temp: val
    })
  },
  getVerify: function(){
    ...
    if(that.data.active){
      wx.request({
        url: 'https://cn2service.ictr.cn/CN2AppWebForWeChatService.asmx/GetMobileCheckCodes',
        data: {
          UserId: '',
  ...
  formSubmit: function(e){
    ...
    console.log(e.detail.value)
    var that = this;
    if (that.data.mobile && that.data.register) {
      wx.request({
        url: 'https://cn2service.ictr.cn//api/WeChatAPI/RegistUserByMobile',
        data: e.detail.value,
        method: 'POST',
        dataType: 'json',
        header: {
